...
DOCUMENTATION = '''
---
module: keycloak_realm_scopemapping

short_description: Allows administration of Keycloak realm_scopemapping with the Keycloak API
version_added: 3.5.0

description:
    - This module allows you to add, remove or modify Keycloak realm_scopemapping with the Keycloak REST API.
    ...
    - When updating a realm_scopemapping, where possible provide the role ID to the module. This removes a lookup
...
options:
    state:
        description:
            - State of the realm_scopemapping.
            - On C(present), the realm_scopemapping will be created if it does not yet exist, or updated with the parameters you provide.
            - On C(absent), the realm_scopemapping will be removed if it exists.
        default: 'present'
        type: str
        choices:
            - present
...
EXAMPLES = '''
- name: Map a realm role to a client scope, authentication with credentials
  community.general.keycloak_realm_scopemapping:
    realm: MyCustomRealm
...
- name: Map a realm role to a client scope, authentication with token
  community.general.keycloak_realm_scopemapping:
    realm: MyCustomRealm
    auth_client_id: admin-cli
    auth_keycloak_url: https://auth.example.com/auth
...
- name: Unmap realm role from a client scope
  community.general.keycloak_realm_scopemapping:
    realm: MyCustomRealm
    auth_client_id: admin-cli
    auth_keycloak_url: https://auth.example.com/auth
...
proposed:
    description: role_representation representation of proposed changes to realm_scopemapping.
    returned: always
    type: dict
    sample: {
      clientId: "test"
...
def main():
    ...
    if roles is None:
        ...
        for role_index, role in enumerate(roles, start=0):
            ...
            if role['id'] is None:
                role_id = kc.get_realm_role(name=role['name'], realm=realm)['id']
            ...
            # Fetch missing role_name
                ...
                role['name'] = kc.get_realm_role_by_id(role['id'], realm=realm)['name']
    ...
    if len(update_roles) or len(delete_roles):
        if state == 'present':
            ...
            if len(update_roles):
                kc.create_clientscope_realm_scopemappings(csid, update_roles, realm=realm)
            if len(delete_roles):
                kc.delete_clientscope_realm_scopemappings(csid, delete_roles, realm=realm)
            result['msg'] = 'Roles %s assigned to client scope %s.' % (update_roles, client_scope_name)
            assigned_roles_after = kc.get_clientscope_realm_scopemappings(csid, realm=realm)
            ...
            if module.check_mode:
                module.exit_json(**result)
            kc.delete_clientscope_realm_scopemappings(csid, update_roles, realm=realm)
